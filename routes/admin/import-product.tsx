...
import { Loader2, Link2, Sparkles, ExternalLink, Copy, CheckCircle2, Upload, Image as ImageIcon } from "lucide-react";
...
function ImportProductPage() {
  ...
  async function uploadImage(file: File) {
    ...
    try {
      ...
      if (!session.session) throw new Error("Please sign in to the admin account before uploading an image.");
      const extension = file.name.split(".").pop()?.toLowerCase() || "jpg";
      const safeSlug = product.slug.trim().replace(/[^a-z0-9-]+/gi, "-").replace(/^-|-$/g, "") || "product";
      ...
      setMessage("Image uploaded successfully. Save Draft or Publish to keep it with the product.");
  ...
  const previewImage = product?.image?.trim() || FALLBACK_IMAGE;
  ...
  return <main className="min-h-screen bg-background px-4 py-10 sm:px-6 lg:px-8"><div className="mx-auto max-w-6xl">
    ...
    {product && <section className="grid gap-6 lg:grid-cols-[1fr_1.35fr]"><div className="rounded-2xl border border-border bg-card p-6 shadow-sm"><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-bold">Imported preview</h2><span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-bold text-primary">{product.network}</span></div><div className="mt-6 overflow-hidden rounded-xl border border-border bg-muted/30"><img src={previewImage} alt={product.title || "Imported product"} className="aspect-square w-full object-contain" onError={(e) => { if (e.currentTarget.src !== new URL(FALLBACK_IMAGE, window.location.origin).href) e.currentTarget.src = FALLBACK_IMAGE; }} /></div><h3 className="mt-5 text-lg font-bold">{product.title || "Product title not found"}</h3>{product.price && <p className="mt-2 text-xl font-bold text-primary">{product.price}</p>}{product.note && <p className="mt-4 rounded-lg bg-muted p-3 text-xs leading-relaxed text-muted-foreground">{product.note}</p>}<div className="mt-4 flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={copyUrl}>{copied ? <CheckCircle2 className="mr-2 size-4" /> : <Copy className="mr-2 size-4" />}{copied ? "Copied" : "Copy affiliate URL"}</Button><Button variant="outline" size="sm" asChild><a href={product.sourceUrl} target="_blank" rel="noopener noreferrer"><ExternalLink className="mr-2 size-4" />Open source</a></Button></div></div>
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm"><div className="mb-6"><h2 className="text-xl font-bold">Review-ready details</h2><p className="mt-1 text-sm text-muted-foreground">Everything is editable before saving or publishing.</p></div><div className="space-y-5"><Field label="Product name" value={product.title} onChange={(v) => update("title", v)} /><Field label="Description" value={product.description} onChange={(v) => update("description", v)} multiline /><div className="grid gap-5 sm:grid-cols-2"><Field label="Price" value={product.price} onChange={(v) => update("price", v)} /><Field label="Rating" value={product.rating} onChange={(v) => update("rating", v)} /><Field label="Category" value={product.category} onChange={(v) => update("category", v)} /><Field label="Slug" value={product.slug} onChange={(v) => update("slug", v)} /></div>
        <div className="space-y-3 rounded-xl border border-border p-4"><div className="flex items-center gap-2"><ImageIcon className="size-4 text-primary" /><span className="text-sm font-semibold">Product image</span></div><div className="grid gap-3 sm:grid-cols-[1fr_auto]"><Input value={product.image} onChange={(e) => update("image", e.target.value)} placeholder="Paste image URL…" /><Button type="button" variant="outline" disabled={uploading} onClick={() => fileInputRef.current?.click()}>{uploading ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Upload className="mr-2 size-4" />}{uploading ? "Uploading…" : "Upload Image"}</Button></div><input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) void uploadImage(file); e.currentTarget.value = ""; }} /><div className="overflow-hidden rounded-lg border border-border bg-muted/30"><img src={previewImage} alt="Product preview" className="h-48 w-full object-contain" onError={(e) => { if (e.currentTarget.src !== new URL(FALLBACK_IMAGE, window.location.origin).href) e.currentTarget.src = FALLBACK_IMAGE; }} /></div><p className="text-xs text-muted-foreground">Upload up to 5 MB (JPG, PNG, WebP or GIF), or paste an image URL. A placeholder is used automatically if the image is missing or broken.</p></div>
        <Field label="Affiliate URL" value={product.sourceUrl} onChange={(v) => update("sourceUrl", v)} /><Field label="Features / specifications" value={product.features.join("\n")} onChange={(v) => update("features", v.split("\n").map((x) => x.trim()).filter(Boolean))} multiline /><Field label="SEO title" value={product.seoTitle} onChange={(v) => update("seoTitle", v)} /><Field label="SEO meta description" value={product.seoDescription} onChange={(v) => update("seoDescription", v)} multiline /><div className="flex flex-col gap-3 border-t border-border pt-5 sm:flex-row"><Button disabled={saving} onClick={() => persist("draft")} className="flex-1">{saving ? <Loader2 className="mr-2 size-4 animate-spin" /> : null}Save Draft</Button><Button disabled={saving} variant="outline" onClick={() => persist("published")} className="flex-1">Publish</Button></div><p className="text-xs text-muted-foreground">Draft and publish use the existing products table and its staff-only write policy.</p></div></div></section>}
  </div></main>;
}
