...
function normaliseLinks(r: Row) { const links = Array.isArray(r.affiliate_links) ? r.affiliate_links : []; return links; }
function normaliseImages(r: Row) { return Array.isArray(r.images) ? r.images.filter((x: unknown) => typeof x === "string" && x.trim()) : []; }
const toProduct = (r: Row, reviewByProductId: Map<string, string>, reviewByProductName: Map<string, string>): Product => ({ id: r.id, slug: r.slug, title: r.title, description: r.description ?? "", images: normaliseImages(r), category: r.category ?? undefined, brand: r.brand ?? undefined, rating: Number(r.rating ?? 0), price: r.price == null ? null : Number(r.price), currency: r.currency ?? "USD", region: r.region ?? "global", affiliateLinks: normaliseLinks(r), reviewSlug: reviewByProductId.get(r.id) ?? reviewByProductName.get(String(r.title).trim().toLowerCase()), status: r.status, seoTitle: r.seo?.title, seoDescription: r.seo?.description, pros: Array.isArray(r.pros) ? r.pros : [], cons: Array.isArray(r.cons) ? r.cons : [], specifications: r.specifications && typeof r.specifications === "object" ? r.specifications : {} });
...
// Keep this projection aligned with the actual public.products schema.
// Do not request legacy/nonexistent fields such as image_url, affiliate_link,
// ai_generated_content, seo_title or seo_description from PostgREST.
