...
function normaliseLinks(r: Row) {
  const links = Array.isArray(r.affiliate_links) ? r.affiliate_links : [];
  if (links.length) return links;
  if (r.affiliate_link) return [{ url: r.affiliate_link, enabled: true, network: r.region === "in" ? "amazon" : "affiliate" }];
  return [];
}

function normaliseImages(r: Row) {
  if (Array.isArray(r.images) && r.images.length) return r.images.filter((x: unknown) => typeof x === "string" && x.trim());
  if (typeof r.image_url === "string" && r.image_url.trim()) return [r.image_url];
  return [];
}
...
async function productList() {
  ...
  const [{ data: products, error: productError }, { data: reviews, error: reviewError }] = await Promise.all([supabase.from("products").select(PRODUCT_COLS).eq("status", "published").order("created_at", { ascending: false }), supabase.from("reviews").select("slug,product,product_ref").eq("status", "published")]);
  if (productError) throw productError; if (reviewError) throw reviewError;
  const reviewByProductId = new Map<string, string>(); const reviewByProductName = new Map<string, string>();
  for (const review of reviews ?? []) { if (review.product_ref) reviewByProductId.set(review.product_ref, review.slug); if (review.product) reviewByProductName.set(String(review.product).trim().toLowerCase(), review.slug); }
...
export const fetchProductBySlug = createServerFn({ method: "GET" }).inputValidator((value: { slug: string }) => value).handler(async ({ data }) => {
  try { const products = await productList(); return products.find((p) => p.slug === data.slug) ?? null; } catch (error) { console.error("fetchProductBySlug failed", error); return null; }
});
