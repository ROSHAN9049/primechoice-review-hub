...
const FUNNELCOCKPIT_AFFILIATE_URL = "https://www.digistore24.com/redir/98201/ROSHANpratibha/";
...
const products: Product[] = [{
  ...
  affiliateUrl: FUNNELCOCKPIT_AFFILIATE_URL,
}];
...
function Digistore24Page() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-2">{filtered.map((product) => <article key={product.id} className="card-surface overflow-hidden rounded-2xl p-5 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><span className="inline-flex rounded-full border border-border bg-secondary px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground">{product.category}</span><h3 className="mt-3 text-xl font-bold leading-tight">{product.name}</h3><p className="mt-3 text-sm leading-relaxed text-muted-foreground">{product.description}</p></div><div className="shrink-0 text-right"><div className="text-lg font-bold text-primary-glow">{product.price}</div><div className="mt-1 text-xs text-muted-foreground">Commission: {product.commission}</div></div></div><div className="mt-6 flex flex-col gap-3 border-t border-border pt-5 sm:flex-row sm:items-center sm:justify-between"><span className="text-xs text-muted-foreground">Digistore24 Product ID: {product.id}</span><a href={product.affiliateUrl} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-primary px-5 py-3 font-display text-sm font-bold uppercase tracking-wide text-primary-foreground transition-opacity hover:opacity-90">View offer <ExternalLink className="size-4" aria-hidden="true" /></a></div></article>)}</div>
