...
export const Route = createFileRoute("/guides/$slug")({
  ...
  head: ({ params, loaderData }) => {
    ...
    return {
      meta: [
        { title: `${g.title} | PrimeChoiceReviews` },
        ...
        { name: "twitter:description", content: g.excerpt },
      ],
...
function GuidePage() {
  ...
  const toc = [
    ...
    ...guide.buyingGuide.map((s) => ({ id: s.id, label: s.heading })),
    { id: "faq", label: "FAQs" },
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <article>
        <header>
          ...
          <p className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
            ...
            <span>· {guide.picks.length} products tested</span>
          </p>
          <img
            src={guide.image}
            alt={guide.title}
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_260px]">
          <div className="min-w-0">
            ...
            <section aria-labelledby="top-picks" id="top-picks" className="mt-12 scroll-mt-24">
              <h2 id="top-picks-heading" className="text-2xl font-bold sm:text-3xl">
                Top picks
              </h2>
              <div className="mt-6 space-y-6">
                {guide.picks.map((p, i) => (
                  <div key={p.name} className="card-surface rounded-2xl p-6">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <Badge className="rounded-full">{p.badge}</Badge>
                    ...
                    <p className="mt-3 text-sm text-muted-foreground">{p.why}</p>
                    ...
                    <div className="mt-6">
                      <AffiliateCtaGroup productName={p.name} {...(p.productId ? { productId: p.productId } : {})} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            >
              <h2 id="comparison-table-heading" className="text-2xl font-bold sm:text-3xl">
                Comparison table
              </h2>
              <div className="mt-5 overflow-x-auto rounded-2xl border border-border">
                <table className="w-full min-w-[560px] text-left text-sm">
                  <caption className="sr-only">Side-by-side comparison of every pick</caption>
                  <thead className="bg-secondary/70">
                    <tr>
                      {guide.specColumns.map((c) => (
                  ...
                  <tbody>
                    {guide.specRows.map((row, i) => (
                      <tr key={row[0]} className={i === 0 ? "bg-primary/5" : "border-t border-border"}>
                        {row.map((cell, j) =>
                          j === 0 ? (
                            <th key={cell} scope="row" className="px-4 py-3 font-semibold">
                              {cell}
                            </th>
                          ) : (
                            <td key={`${row[0]}-${j}`} className="px-4 py-3 text-muted-foreground">
                              {cell}
                            </td>
                          ),
                        )}
            ...
            <section aria-labelledby="faq-heading" id="faq" className="mt-14 scroll-mt-24">
            ...
            >
              <h2 id="final-recommendation-heading" className="text-2xl font-bold sm:text-3xl">
                Final recommendation
              </h2>
              <p className="mt-3 text-muted-foreground">{guide.recommendation}</p>
              <div className="mt-6 max-w-md">
                <AffiliateButton
                  label={`Check price on ${guide.picks[0]?.name ?? "our top pick"}`}
      ...
      <section aria-labelledby="related-reviews" className="mt-16">
        <h2 id="related-reviews" className="text-2xl font-bold">
          Related reviews
        </h2>
      ...
      <section aria-labelledby="related-comparisons" className="mt-16">
        <h2 id="related-comparisons" className="text-2xl font-bold">
          Related comparisons
        </h2>
        <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
