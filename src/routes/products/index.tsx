...
function ProductImage({ product }: { product: PublicProduct }) {
  ...
  useEffect(() => {
    setSrc(candidate && !candidate.includes("via.placeholder.com") ? candidate : fallbackImage(product.slug));
...
function ProductsPage() {
  const [products, setProducts] = useState<PublicProduct[]>([]); const [loading, setLoading] = useState(true); const [query, setQuery] = useState("");
  useEffect(() => { let active = true; void supabase.from("products").select("id,slug,title,description,images,category,brand,rating,price,currency,region,affiliate_links,status").eq("status", "published").order("created_at", { ascending: false }).then(({ data }) => { if (active) { setProducts((data ?? []).map((row) => mapProduct(row as Record<string, unknown>))); setLoading(false); } }); return () => { active = false; }; }, []);
  const filtered = useMemo(() => { const term = query.trim().toLowerCase(); return term ? products.filter((p) => `${p.title} ${p.category ?? ""} ${p.brand ?? ""}`.toLowerCase().includes(term)) : products; }, [products, query]);
