...
export const Route = createFileRoute("/reviews/$slug")({
  ...
  head: ({ params, loaderData }) => {
    ...
    return {
      meta: [
        { title: r.title }, { name: "description", content: r.excerpt },
        { property: "og:title", content: r.title }, { property: "og:description", content: r.excerpt },
        { property: "og:type", content: "article" }, { property: "og:url", content: `/reviews/${params.slug}` },
        { name: "twitter:title", content: r.title }, { name: "twitter:description", content: r.excerpt },
      ],
      links: [{ rel: "canonical", href: `/reviews/${params.slug}` }],
      scripts: [
        { type: "application/ld+json", children: JSON.stringify({ "@context": "https://schema.org", "@type": "Product", name: r.product, brand: { "@type": "Brand", name: r.vendor }, description: r.excerpt, review: { "@type": "Review", name: r.title, reviewRating: { "@type": "Rating", ratingValue: r.rating, bestRating: 5 }, author: { "@type": "Organization", name: "PrimeChoiceReviews" }, datePublished: r.updated } }) },
        { type: "application/ld+json", children: JSON.stringify({ "@context": "https://schema.org", "@type": "FAQPage", mainEntity: r.faq.map((f) => ({ "@type": "Question", name: f.question, acceptedAnswer: { "@type": "Answer", text: f.answer } })) }) },
        { type: "application/ld+json", children: JSON.stringify(breadcrumbSchema([{ label: "Home", to: "/" }, { label: "Reviews", to: "/reviews" }, { label: r.product }])) },
      ],
    };
  },
...
function ReviewPage() {
  ...
  const categoryImage = `/category-art-${review.category}.svg`;
      ...
      <article>
        <header className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <h1 className="mt-4 text-3xl leading-tight font-extrabold sm:text-4xl">{review.title}</h1>
            <p className="mt-4 text-lg text-muted-foreground">{review.excerpt}</p>
            <p className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground"><span className="inline-flex items-center gap-1.5"><RefreshCw className="size-3.5" aria-hidden="true" />Updated {new Date(review.updated).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</span><span>· Editorial review</span></p>
          ...
          <div className="card-surface rounded-2xl p-6">
            <img src={categoryImage} alt={`${category?.name ?? "Category"} review guide`} width={1024} height={768} className="aspect-[4/3] w-full rounded-2xl object-cover" onError={(e) => { e.currentTarget.src = "/product-placeholder.svg"; }} />
