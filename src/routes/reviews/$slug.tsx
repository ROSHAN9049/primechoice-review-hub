...
function ReviewPage() {
  ...
  useEffect(() => {
    trackEvent({
      ...
      refSlug: review.slug,
      network: "Digistore24",
      meta: { productId: review.productId ?? null, product: review.product },
    });
  }, [review.product, review.productId, review.slug]);
    ...
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <article>
        <header className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="card-surface rounded-2xl p-6">
            ...
            <div className="mt-5 flex items-center justify-between gap-4">
              <div>
                <p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Overall rating</p>
                <p className="font-display text-4xl font-extrabold text-primary">
                  {review.rating.toFixed(1)}<span className="text-base text-muted-foreground">/5</span>
                </p>
              </div>
        ...
        <section aria-labelledby="verdict" className="mt-14">
          <h2 id="verdict" className="text-2xl font-bold sm:text-3xl">Review summary</h2>
          <div className="mt-4 space-y-4 text-muted-foreground">
            {review.summary.map((p) => <p key={p.slice(0, 24)}>{p}</p>)}
          </div>
        </section>

        <section aria-labelledby="proscons" className="mt-14">
          <h2 id="proscons" className="text-2xl font-bold sm:text-3xl">Pros & cons</h2>
          <div className="mt-5 grid gap-5 sm:grid-cols-2">
            <div className="card-surface rounded-xl p-6">
              <h3 className="font-semibold text-success">What we liked</h3>
              <ul className="mt-4 space-y-2.5">
                {review.pros.map((p) => <li key={p} className="flex gap-2.5 text-sm"><Check className="mt-0.5 size-4 shrink-0 text-success" aria-hidden="true" /><span>{p}</span></li>)}
              </ul>
            </div>
            <div className="card-surface rounded-xl p-6">
              <h3 className="font-semibold text-destructive">What could be better</h3>
              <ul className="mt-4 space-y-2.5">
                {review.cons.map((c) => <li key={c} className="flex gap-2.5 text-sm"><X className="mt-0.5 size-4 shrink-0 text-destructive" aria-hidden="true" /><span>{c}</span></li>)}
              </ul>
            </div>
          </div>
        </section>
        ...
        <section aria-labelledby="features" className="mt-14">
          <h2 id="features" className="text-2xl font-bold sm:text-3xl">Key features</h2>
