  ...
  return (
    <section id="emags" aria-labelledby="emags-heading">
      <div className="rule-line flex flex-wrap items-end justify-between gap-4 pt-6">
        ...
        <Link to="/digistore24" className="group inline-flex items-center gap-1.5 font-display text-sm font-bold tracking-wide text-primary-glow uppercase">View all <ArrowUpRight className="size-4" aria-hidden="true" /></Link>
      </div>
      <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {featuredEmags.map((item) => {
          const category = getCategory(item.slug);
          const artwork = editorialArtwork(item.slug);
          return (
            <Link key={item.slug} to="/guides/$slug" params={{ slug: item.guideSlug }} className="card-surface group overflow-hidden rounded-2xl transition-all duration-300 hover:-translate-y-1 hover:border-primary/30 hover:shadow-elevated">
              <div className="relative h-48 overflow-hidden bg-muted">
                <img src={artwork} alt={`${category?.name ?? item.name} category`} width={1024} height={675} loading="eager" decoding="async" fetchPriority="high" className="block h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.03]" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = "/category-art-default.svg"; }} />
                <span className="absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-background/85 px-2.5 py-1 text-[10px] font-bold tracking-[0.14em] text-foreground uppercase backdrop-blur"><BookOpen className="size-3" aria-hidden="true" /> Guide</span>
              </div>
              <div className="p-5">
                <h3 className="font-display text-lg font-bold tracking-tight transition-colors group-hover:text-primary-glow">{item.name}</h3>
                <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{item.description}</p>
                <span className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-primary-glow">Read guide <ArrowUpRight className="size-3.5" aria-hidden="true" /></span>
              </div>
            </Link>
          );
        })}
