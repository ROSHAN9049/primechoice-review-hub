...
export function ScoreBadge({ rating, className }: { rating: number; className?: string }) {
  return (
    <span
      className={cn(
        "inline-flex items-baseline gap-1 rounded-md px-2.5 py-1 font-display text-sm font-bold text-primary-foreground shadow-soft",
        className,
      )}
...
export function ReviewCard({ review, index }: { review: Review; index?: number }) {
  ...
  return (
    <article
      className="card-surface group relative flex h-full flex-col overflow-hidden rounded-xl hover:-translate-y-1.5 hover:border-primary/30 hover:shadow-elevated"
      style={index !== undefined ? { animationDelay: `${index * 70}ms` } : undefined}
    >
      <Link
        to="/reviews/$slug"
      ...
      >
        ...
        />
        <span className="absolute top-3 left-3 rounded-md bg-background/85 px-2.5 py-1 font-display text-[10px] font-bold tracking-[0.14em] text-foreground uppercase backdrop-blur-sm">
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        <h3 className="font-display text-[1.05rem] leading-snug font-bold tracking-tight">
          <Link to="/reviews/$slug" params={{ slug: review.slug }} className="editorial-underline">
